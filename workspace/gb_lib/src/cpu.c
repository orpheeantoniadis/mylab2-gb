...
void print_registers(void) {
  printf("AF : 0x%04x\n"
         ...
         registers.pc, FLAG_ISSET(ZERO_FLAG) ? 1 : 0,
         FLAG_ISSET(NEGATIVE_FLAG) ? 1 : 0, FLAG_ISSET(HALFCARRY_FLAG) ? 1 : 0,
         FLAG_ISSET(CARRY_FLAG) ? 1 : 0);
}
...
void print_instruction(void) {
  ...
  case 0:
    printf("0x%02x : %s (0x%02x)\n", registers.pc,
           instruction_set[opcode].disassembly, opcode);
    break;
  case 1:
    n = read8(registers.pc + 1);
    if (opcode == 0xcb) {
      printf("0x%02x : %s (0x%02x 0x%02x)\n", registers.pc,
             prefix_cb[n].disassembly, opcode, n);
    } else {
